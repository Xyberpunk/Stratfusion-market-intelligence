...
class SentimentFeatureBuilder:
    ...
    def build(self, frame: pd.DataFrame, events: list[NewsSentimentEvent]) -> pd.DataFrame:
        ...
        joined["sentiment_count"] = joined["sentiment_count"].astype(int)
        ...
        joined["bullish_headline_count"] = joined["bullish_count"].rolling(5, min_periods=1).sum().astype(int)
        joined["bearish_headline_count"] = joined["bearish_count"].rolling(5, min_periods=1).sum().astype(int)
