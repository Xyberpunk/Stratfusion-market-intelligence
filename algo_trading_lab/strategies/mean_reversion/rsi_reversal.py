...
class RSIReversalStrategy(BaseStrategy):
    ...
    def generate(self, context: StrategyContext) -> StrategySignal:
        ...
        rsi = float(context.features.iloc[-1].get("rsi", context.features.iloc[-1].get("rsi_14", 50.0)))
        if rsi < 30:
        ...
        if 45 <= rsi <= 55:
            return self._signal(context, 0.0, 0.45, "RSI is between 45 and 55, so reversal edge is neutral.", {"rsi": rsi})
